...
{
	const std::string modelPach(FileName);
...
void AnimationModel::LoadAnimation(const char* FileName, const char* AniName)
{
	m_Animation[AniName] = aiImportFile(FileName, aiProcess_ConvertToLeftHanded);
	assert(m_Animation[AniName]);
}
...
void AnimationModel::Unload()
{
	...
	//for (auto pair : m_Texture) ← autoにするとコンパイラが自動で型を判断してくれる
...
void AnimationModel::Update(int frame, const char* Aniname)
{
	if (!m_Animation[Aniname]->HasAnimations())
		return;

	aiAnimation* animation = m_Animation[Aniname]->mAnimations[0];
...
void AnimationModel::Update(int frame, const char* Aniname1, const char* Aniname2, float BlendRate)
{
	if (!m_Animation[Aniname1]->HasAnimations())
		return;

	if (!m_Animation[Aniname2]->HasAnimations())
		return;

	aiAnimation* animation1 = m_Animation[Aniname1]->mAnimations[0];
	aiAnimation* animation2 = m_Animation[Aniname2]->mAnimations[0];
...
/*
	if( == aiMatrix4x4("RightHand"))

	D3DXMATRIX mat;

	mat._11 = bone->Matrix.a1;*/
...
DEFORM_VERTEX AnimationModel::GetDEFORM_VERTEX()
{
	DEFORM_VERTEX p;
	p = m_DeformVertex->at(0);
